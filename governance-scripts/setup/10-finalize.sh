...
printf "  %-36s %s\n" "Installation:"         "Any account  ← required so it can be installed on ${GITHUB_ORG} too"
echo ""
...
echo -e "  ${CYAN}Governance org (already done after creation):${NC}"
...
echo ""
echo -e "  ${CYAN}Engineering org — two sub-steps:${NC}"
echo -e "  ${BOLD}First${NC} — change install scope to 'Any account':"
echo -e "  https://github.com/organizations/${PLATFORM_ORG}/settings/apps/${PLATFORM_ORG}-bot"
echo -e "  → Scroll to bottom → 'Where can this app be installed?' → Any account → Save changes"
echo ""
echo -e "  ${BOLD}Then${NC} — install on ${GITHUB_ORG}:"
echo -e "  https://github.com/organizations/${PLATFORM_ORG}/settings/apps/${PLATFORM_ORG}-bot/installations"
echo -e "  → Click Install next to ${GITHUB_ORG} → All repositories → Install"
echo ""
...
printf "  %-42s %s\n" "ENTERPRISE_ADMIN_TOKEN"      "PAT (classic) with admin:enterprise scope — required for new org provisioning (step 5g)"
echo ""
echo -e "  ${DIM}ENTERPRISE_ADMIN_TOKEN must be owned by a GitHub Enterprise owner.${NC}"
echo -e "  ${DIM}Classic PAT scopes required: admin:enterprise${NC}"
...
echo ""
echo -e "${BOLD}━━━ REPOSITORY VARIABLES: Governance Repo ━━━${NC}"
echo -e "${DIM}→ $GOV_URL/settings/variables/actions${NC}"
echo ""
printf "  %-42s %s\n" "VARIABLE NAME" "VALUE"
divider
printf "  %-42s %s\n" "GITHUB_ORG_NAME"          "$GITHUB_ORG"
printf "  %-42s %s\n" "GITHUB_ENTERPRISE_SLUG"   "$GITHUB_ENTERPRISE  ← set automatically by step 5g"
echo ""
echo -e "  ${DIM}GITHUB_ENTERPRISE_SLUG is set automatically when step 5g runs.${NC}"
echo -e "  ${DIM}Verify at: $GOV_URL/settings/variables/actions${NC}"
...
if $TWO_ORG_MODE; then
  ...
  echo ""
  echo -e "  ${CYAN}Governance repo → New orgs (via org provisioning):${NC}"
  echo -e "  ENTERPRISE_ADMIN_TOKEN is used by org-bootstrap.yml to call"
  echo -e "  POST /enterprises/{slug}/organizations — creating the org and applying policies."
  echo -e "  After provisioning, the meridian-governance-bot must be manually installed"
  echo -e "  in each new org for ongoing cross-org operations."
  echo ""
...
echo ""
divider
echo -e "  ${BOLD}7.${NC}  Request a new repo (issue UI, recommended):"
echo -e "       ${CYAN}https://github.com/${PLATFORM_ORG}/${GOVERNANCE_REPO}/issues/new?template=repo-request.yml${NC}"
echo -e "       Engineers fill the form → standard tier auto-provisions,"
echo -e "       elevated/critical get routed to a platform-admin for /approve."
echo -e "  ${BOLD}8.${NC}  Request a new repo (CLI fallback):"
echo -e "       ${CYAN}\$ bash governance-scripts/factory/new-repo.sh${NC}"
echo -e "  ${BOLD}9.${NC}  Test the team factory:   ${CYAN}\$ bash governance-scripts/factory/new-team.sh${NC}"
echo -e "  ${BOLD}10.${NC} Trigger compliance report:"
echo -e "       ${CYAN}\$ gh workflow run waiver-report.yml --repo ${PLATFORM_ORG}/${GOVERNANCE_REPO}${NC}"
echo -e "  ${BOLD}11.${NC} Trigger audit report:"
echo -e "       ${CYAN}\$ gh workflow run central-audit-report.yml --repo ${PLATFORM_ORG}/${GOVERNANCE_REPO}${NC}"
echo -e "  ${BOLD}12.${NC} Verify a real PR in an engineering repo has all compliance checks running."
echo -e "  ${BOLD}13.${NC} Brief your teams on waiver requests, repo requests, and compliance reports."
echo ""
echo -e "  ${BOLD}Self-service URLs for engineers:${NC}"
echo -e "  📦 New repo:    ${CYAN}https://github.com/${PLATFORM_ORG}/${GOVERNANCE_REPO}/issues/new?template=repo-request.yml${NC}"
echo -e "  🔐 Access req:  ${CYAN}https://github.com/${PLATFORM_ORG}/${GOVERNANCE_REPO}/issues/new?template=access-request.yml${NC}"
echo -e "  📋 Waiver:      ${CYAN}https://github.com/${PLATFORM_ORG}/${GOVERNANCE_REPO}/issues/new?template=waiver-request.yml${NC}"
echo ""
